...
namespace Lab4Try2
{
    public class Await
    {
        ...
        static async void ReceiveResponse(StateObject so)
        {
        ...
        static async void SendRequest(StateObject so)
        {
            Socket s = so.workSocket;
            so = await Task.Run(() =>
            {
        ...
        static async void Connect(StateObject so)
        {
            Socket s = so.workSocket;
            so = await Task.Run(() =>
            {
